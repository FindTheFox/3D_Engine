...
/*   Updated: 2020/05/08 03:31:52 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void            thread_init(t_env *e, t_thread *thread, t_dyntab *to_raster)
{
    int todo;
    int rest;
    todo = (to_raster->cell_nb / NB_THREAD);
    rest = (to_raster->cell_nb % NB_THREAD);
    thread->env = e;
    thread->mode = e->usr.mode;
    thread->tris = *to_raster;
...
void            rasterizer(t_env *e, t_dyntab *to_raster)
{
    t_triangle  *t;
    ...
    {
        thread[i].id = i;
        thread_init(e, &thread[i], to_raster);
        ++i;
    }
    i = 0;
    while (i < NB_THREAD)
